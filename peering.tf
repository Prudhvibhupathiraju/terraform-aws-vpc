resource "aws_vpc_peering_connection" "peering-test" {
...
resource "aws_route" "acceptor_route" {
  ...
  destination_cidr_block    = var.aws_vpc
  vpc_peering_connection_id = aws_vpc_peering_connection.peering-test[0].id
}
